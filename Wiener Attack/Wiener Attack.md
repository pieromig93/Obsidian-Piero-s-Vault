Vediamo prima le basi matematiche fornite da Wiener sulle quali si fonda questo tipo di attacco.
...
![[Pasted image 20230303103610.png]]
...
![[Pasted image 20230303120254.png]]
...
![[Pasted image 20230303152922.png]]
e quindi, moltiplicando il tutto per $p$ otteniamo una equazione di secondo grado che sappiamo risolvere tranquillamente, trovando le radici $p_1 \ \text{e} \ p_2$ che compongono $N$.
...
Ricordando che $$\displaylines{ed -k\phi(N) = 1}$$ e compiendo dei calcoli banali algebrici, è possibile scrivere che $$\frac{e}{\phi(N)} - \frac{k}{d} = \frac{1}{d\phi(N)}$$
...
- cercare i convergenti dell'espansione in continued fractions di $\frac{e}{N}$
- iterare attraverso i convergenti di $\frac{d_i}{k_i}$
	- stimare il totiente $\phi_i(N)$ usando $k_i$ e $d_i$ 
	- verificare se il totiente è corretto valutando l'equazione quadratica vista in precedenza

Per verificare quest'ultimo punto 